...
Ngôn ngữ và nền tảng chính: React, TypeScript, NestJS, PostgreSQL, Socket.IO, Dify AI
...
Ở frontend, file `.env` chủ yếu khai báo `VITE_API_URL` và các cấu hình liên quan đến Google Sign-In. Việc tách cấu hình theo môi trường giúp project thuận lợi khi chạy local, build production hoặc triển khai trên Vercel và Fly.io.
...
Một điểm cần lưu ý là mã nguồn hiện tại chưa tích hợp `@nestjs/swagger`, vì vậy hệ thống chưa có tài liệu Swagger tự động cho API. Trong giai đoạn thực nghiệm, việc kiểm tra API được thực hiện thông qua các service frontend, lời gọi HTTP nghiệp vụ và các script test đã có. Đây cũng là một hướng hoàn thiện phù hợp ở giai đoạn phát triển tiếp theo để tăng tính trực quan khi kiểm thử và bàn giao hệ thống.
